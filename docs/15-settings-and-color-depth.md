...
- **Depths the encoder produces:** `pict::Depth` = **1 / 4 / 8** (indexed,
  adaptive median-cut palettes; 1-bit via ordered Bayer dither) and **16 / 24**
  (direct — 16-bit 1-5-5-5 "Thousands", 24-bit 8-8-8 "Millions" in 32-bit pixels).
  Verified across depths in Snow (each frame self-labels via the Settings panel):
  [1-bit](evidence/depth-compare-1bit.png) · [4-bit](evidence/depth-compare-4bit.png)
  · [8-bit](evidence/depth-compare-8bit.png) ·
  [24-bit/Millions](evidence/depth-compare-24bit-millions.png).
- **Best-variant selection + down-conversion.** `load_item_art` picks the best
  available variant for the screen: the native depth, then *higher* depths
  (QuickDraw down-converts a deeper PICT to the screen — verified, no crash), then
  *lower* colour depths, with the 1-bit raw last. So:
    - a **single deep master** (`["1","24"]`) covers every colour screen — the
      24-bit PICT down-converts to 8/4/2-bit, and 1-bit uses the dithered raw.
      Simplest/fewest files; the catch is that QuickDraw maps indexed screens to
      the *system* CLUT, so an 8-bit screen looks slightly worse than a native
      adaptive-palette `.8.pict`.
    - **per-depth colour variants** (`["1","8","24"]` or `["1","4","8","24"]`) give
      each indexed screen its own adaptive palette (best quality) plus true colour
      on Millions; more/larger files. `load_item_art` still down-converts to fill
      any depth without an exact variant (e.g. a 4-bit screen with only `1/8/24`
      now uses 8→4 down-convert, not the 1-bit raw).
  1-bit always prefers the ordered-dither `<id>.1.raw`
  ([evidence/depth-art-1bit-bw.png](evidence/depth-art-1bit-bw.png)) over QuickDraw's
  coarse threshold down-convert
