...
**Phase 2** — [x] `bless.c` (enumerate + `PBSetVInfo`; de-risked vs `rb-cli bless`) · [x] chooser UI (built-in widgets, Quick-Launch + Special menu) · [x] Snow verify swap (7.1.2 → 6.0.8) · [x] per-folder System version + MacOS-version header · [x] compatibility gating (gray incompatible + swap warning, docs/40) · [x] **host per-System startup placement** (`install_all_systems` config + `atrium install-all-systems`) · [ ] filter/handle pre-6 Systems in the chooser (build now *skips* them; launcher still shows them enabled)
